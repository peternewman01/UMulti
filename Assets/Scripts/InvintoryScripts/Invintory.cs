...
public class Invintory : MonoBehaviour
{
    ...
    public bool RemoveItem(int id, int count)
    {
        if (Stuff.ContainsKey(id))
        {
            ...
            {
                Slot HoldSlot;
                cp.RemoveObject(ItemManager.GetItem(id), removedSlots, out HoldSlot);
                removedSlots.Add(HoldSlot);
            }
    ...
    public void SetControlPannel(ControlPanel controlPanel) { cp = controlPanel; }
